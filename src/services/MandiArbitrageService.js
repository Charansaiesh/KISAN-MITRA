...
class MandiArbitrageService {
  ...
  static calculateArbitrage({
    ...
    if (candidateMandis.length === 0) {
      // Fallback to top 5 closest if all are further
      candidateMandis = mandiEvaluations.sort((a, b) => a.distance_km - b.distance_km).slice(0, 5);
    }
    ...
    return {
      ...
      comparison: sortedByProfit.slice(0, 6) // Return top 6 compared mandis
    };
  }
}
